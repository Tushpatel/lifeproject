...
public final class FusumaViewController: UIViewController {
    ...
    public var isCamera = false //rb - my change (added)
...
//    }
    ...
    var image: UIImage? = nil //rb - my change
...
//        }
    ...
    override public func viewWillAppear(_ animated: Bool) {
        ...
        if isCamera {
            cameraView.startCamera() //rb - my change (added)
        }
    ...
    @IBAction func closeButtonPressed(_ sender: UIButton) {
        //rb - my change begin (removed)
//        self.dismiss(animated: true, completion: {
//            
//            self.delegate?.fusumaClosed?()
//        })
        //rb - my change end (removed)
        
        //rb - my change begin (added)
        if AppManager.shared().fusumaScreen == 1 {
            tabBarController?.selectedIndex = 0 // go to first tab if user is on second tab
        }
        else {
            _ = navigationController?.popViewController(animated: true) // go to current tab
        }
        //rb - my change end (added)
    }
...
//                        })
                        ...
                        print("... \(AppManager.shared().fusumaScreen)")
                        
                        if AppManager.shared().fusumaScreen == 1 {
                            self.performSegue(withIdentifier: "SegueToFiltersVC", sender: self)
                        }
                        else if AppManager.shared().fusumaScreen == 2 {
                            self.performSegue(withIdentifier: "SegueToFinalImageVC", sender: self)
                        }
                        else if AppManager.shared().fusumaScreen == 3 {
                            self.performSegue(withIdentifier: "SegueToPPFinalVC", sender: self)
                        }
                        //rb - my change end (added)
                    })
                }
            ...
            print("no image crop ")
//            delegate?.fusumaImageSelected((view?.image)!) //rb - my change (removed)
...
extension FusumaViewController: FSAlbumViewDelegate, FSCameraViewDelegate, FSVideoCameraViewDelegate {
    ...
    override public func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "SegueToFiltersVC" {
            let nextScene =  segue.destination as! FiltersViewController
            nextScene.image = image
        }
        else if segue.identifier == "SegueToFinalImageVC" {
            let nextScene =  segue.destination as! FinalImageViewController
            nextScene.finalImage = image
        }
        else if segue.identifier == "SegueToPPFinalVC" {
            let nextScene =  segue.destination as! PPFinalViewController
            nextScene.finalImage = image
        }
    }
    //rb - my change end (added)
...
private extension FusumaViewController {
    ...
    func changeMode(_ mode: Mode) {
        ...
        case .library:
            ...
            isCamera = false //rb - my change (added)
        case .camera:
            ...
            isCamera = true //rb - my change (added)
        case .video:
...
//        doneButton.isHidden = !hasGalleryPermission //rb - my change (removed)
